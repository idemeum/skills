...
`Condition:` only run if (a) Step 3's `get_network_interfaces` shows the active interface has `type: "Wi-Fi"`, (b) Step 4's `get_wifi_info` returned a non-null, non-empty `ssid` (see SSID-unavailable fallback below for the macOS 14.4+ case where `ssidAvailable: false`), and (c) Steps 5–8 did not resolve the issue. Skip silently otherwise — do NOT invent an SSID or call with an empty value.
