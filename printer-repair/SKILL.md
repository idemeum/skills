...
Call `list_printers`. Returns `output.printers: [{ name, status, type, host, queueDepth }]` and `output.defaultPrinter`. `status` is canonical across platforms: `idle | processing | stopped | disabled | offline | error | unknown` — treat `unknown` as "state undetermined; do NOT assume healthy". `type` is canonical: `network | local | virtual`. `host` is the printer's IP/hostname when derivable (network printers); `null` for USB/virtual printers and for Bonjour printers whose host isn't literal in the device URI.

**Step 2 — Check all print queues**
Call `check_print_queue` with **no `printerName`**. Returns `output.jobs: [{ id, printer, owner, document, status, sizeKb, sizeHuman, submittedAt }]`, `output.stuckCount` (jobs in error/paused/`held` state — `held` = queued behind a stopped/disabled printer), and `output.total`. Substitute `sizeHuman` verbatim; do NOT recompute from `sizeKb`.
...
It tests ports 9100 (raw), 631 (IPP), 80 (admin). If all are unreachable, the printer is off/asleep or on a different IP. If 631 is reachable (`output.isReachable`), the printer is online and the issue is software-side.

**Step 5 — Clear stuck jobs (lightest corrective)**
**Condition:** only if Step 2 returned `output.total > 0`.
Call `clear_print_queue`. G4's dry-run preview + consent gate fires automatically (the user sees which jobs would be cancelled). Admin-gated → runs via the helper daemon; if the helper is unavailable, see Privilege handling for the self-service queue-clear.
...
- **macOS CUPS restrictions (Sequoia+)** — `restart_cups` may require Full Disk Access approval in System Settings → Privacy & Security if the helper isn't handling it.
