...
Call `c_intune_find_device` — no parameters, the serial comes from the runtime. Check `matchCount` first: continue ONLY when it is exactly 1. `0` means not in this tenant; `>1` means the serial is ambiguous (VM templates and some OEM batches ship duplicates) so the record may be a different machine — **act on none of them** and say IT must identify the right one. Then check `lastSyncDateTime`. If the device last contacted Intune more than **7 days** ago it is not collecting policy at all, so a re-sync will sit unacknowledged and the wait would be spent for nothing — skip Steps 14–17, report the stale check-in date as the finding, and escalate. That date is more useful to IT than any symptom: it says the device fell off management, not that the agent is misconfigured.
...
`Condition:` only run if Step 14 shows a policy in error / non-compliant state matching the diagnosed fault. Skip when every policy applied cleanly — the fault is local and Steps 2–8 already addressed what they could.
